  ...
  provideState({
    computed: {
      ...
      async backups(_, { remotes = [] }) {
        ...
        const { xo, pool } = await listMetadataBackups(remotes)

        const collection = {}
        forOwn(xo, entries =>
          entries.forEach(entry => {
            const jobName = entry.jobName
            let backup = collection[jobName]
            if (backup === undefined) {
              backup = collection[jobName] = {
                backups: [],
                id: `Xen Orchestra(${jobName})`,
                label: jobName,
                type: 'XO',
              }
            }
        ...
        forOwn(pool, entriesByRemote =>
          forOwn(entriesByRemote, (entriesByPool, poolId) => {
            let backup = collection[poolId]
            if (backup === undefined) {
              const { pool, poolMaster } = entriesByPool[0]
